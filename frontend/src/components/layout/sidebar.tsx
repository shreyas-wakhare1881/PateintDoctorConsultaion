...
/**
 * Sidebar — collapsible navigation for desktop dashboard.
 * Source of truth: frontend/SDD/admin.md — Responsive Design Notes
 *
 * Desktop: fixed left w-64. Mobile: hidden (replaced by MobileBottomNav).
 * UI: Dark slate bg, pill active state, icon accent. Routes/nav unchanged.
 */

import Link from 'next/link';
import { usePathname } from 'next/navigation';
import { cn } from '@/utils/cn'; // used for nav link className
...
const PATIENT_NAV: NavItem[] = [
  ...
  },
  {
    ...
    icon: <svg className="h-[18px] w-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" /></svg>,
  },
  {
    label: 'Consultations',
  ...
  },
  {
    ...
    icon: <svg className="h-[18px] w-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" /></svg>,
  },
];
    ...
    >
      ...
      <div className="flex items-center gap-2.5 px-5 py-5" style={{ borderBottom: '1px solid #1E293B' }}>
        <div
          ...
          style={{ width: 32, height: 32, background: 'linear-gradient(135deg, #304F6D, #899481)' }}
        >
          <div className="absolute rounded-full bg-white" style={{ width: 7, height: 18 }} />
          <div className="absolute rounded-full bg-white" style={{ width: 18, height: 7 }} />
        </div>
        <div>
          <div className="font-bold leading-none tracking-tight text-white" style={{ fontSize: 15 }}>
            HealthConsult
          </div>
          <div className="leading-none mt-0.5" style={{ fontSize: 9, color: '#475569' }}>
      ...
      {/* Nav items */}
      <nav className="flex flex-1 flex-col gap-0.5 px-3 mt-1" aria-label="Main navigation">
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              style={
                isActive
                  ? {
                      background: 'rgba(48,79,109,0.12)',
                      color: '#304F6D',
                      borderLeft: '3px solid #304F6D',
                      paddingLeft: '9px',
                    }
                  : {
                      color: '#94A3B8',
            ...
            >
              <span style={{ color: isActive ? '#304F6D' : '#64748B', flexShrink: 0 }}>
                {item.icon}
              </span>
              {item.label}
            </Link>
      ...
      <div className="p-3" style={{ borderTop: '1px solid #1E293B' }}>
        <button
          ...
          style={{ color: '#64748B', paddingLeft: '9px' }}
          onMouseEnter={(e) => {
