...
export function MobileBottomNav() {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-50 flex h-16 items-center md:hidden"
      style={{
      ...
      {items.map((item) => {
        ...
        return (
          <Link
            key={item.href}
            href={item.href}
            className="flex flex-1 flex-col items-center justify-center gap-0.5 py-2 text-[10px] font-semibold transition-all duration-200"
            style={{ color: isActive ? '#304F6D' : '#6B7280', letterSpacing: '-0.01em' }}
            aria-current={isActive ? 'page' : undefined}
          >
            {/* Active indicator */}
            <span
              className="mb-0.5 h-1 w-4 rounded-full transition-all duration-200"
              style={{ background: isActive ? '#E07D54' : 'transparent' }}
            />
            {item.icon}
            {item.label}
          </Link>
